...
\begin{figure}
  \begin{subfigure}[t]{0.5663\linewidth}
    ...
    \caption{Front view of the CFRP plate during LDV scanning. The white speckle coating increases surface reflectivity.}
    \label{fig:Setup:a}
  \end{subfigure}\hfill
  \centering
  \begin{subfigure}[t]{0.4235\linewidth}
    ...
    \caption{Back view of the plate showing the five transducers. The 1\,cm$^3$ metal cube (defect) is bonded on the rear side; the central square mark is residue from prior tests.}
    \label{fig:Setup:b}
  \end{subfigure}
  
  \caption{Experimental setup for CFRP plate measurements.}
  \label{fig:Setup}
\end{figure}
...
We first present a baseline simulation (Case A) to illustrate the method under controlled conditions. We then provide experimental validation using the experimental setup described (see Section~\ref{Sec:Exp}). Finally, we present additional simulations that probe the limits and variants of the approach.

\subsection{Results - Simulations (Case A)}\label{Simulation_Results}
The method was validated using the spectral-element modeling suite Salvus \cite{mondaic}. We simulated elastic wave propagation on a 50\,cm $\times$ 50\,cm, 9\,mm-thick rectangular aluminum plate. The defect was modeled as localized density reduction. Figure \ref{fig:Simulation_A_Defect} shows a tapered circular lower-density inclusion of 1.5 cm diameter with relative density 20\%. The driving pulse was a 50\,kHz central-frequency Ricker wavelet with a 2\,ms repetition period. The configuration used 5 sensors, 20\% Gaussian RMS noise, and a 0--50\,kHz low-pass filter. This baseline case serves as a reference for comparison with subsequent results.

Figure~\ref{fig:Simulation_A_results} shows the reconstructed defect vector $|\vec{\mathbf{d}}|$, visualized as a heatmap of its magnitude at each spatial grid point ( measurement location). Solving equation~\ref{full_system} yields a complex-valued coefficient at every grid point, interpreted here as a local reflection amplitude. In this experiment, $\vec{\mathbf{d}}$ contains $100\times100 = 10,000$ entries. These are mapped back to the original 2D receiver grid for visualization. Since the plot shows $|\vec{\mathbf{d}}|$, phase information is not displayed.
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{Simulation case A: comparison between reconstructed defect amplitude and ground-truth density.}
...
\subsection{Results - Experimental Validation}\label{Results_Experimental}
...
\paragraph{}
To probe the low-frequency limit, we also evaluate the 1--15\,kHz band (audible range). Running the calculation on the full $104\times104$ grid yields mixed results (Appendix~\ref{Appendix_15khz}). We therefore reduce the reconstruction grid to $50\times50$ by using the same dataset as above but restricting the inversion to a subgrid. The corresponding reconstruction is shown in Figure~\ref{fig:15khz_subsampled} and yields a compact defect signature at 15\,kHz with an estimated FWHM of $\approx1.4$\,cm, despite a wavelength on the order of $\approx16$\,cm in this band.
...
\begin{figure}
  \begin{subfigure}[t]{0.48\linewidth}
    ...
    \caption{Reconstructed magnitude of the defect vector $|\vec{\mathbf{d}}|$ in the 1--50 kHz band for the experimental CFRP plate. The adhered metal cube defect is clearly detected near its true location (yellow square). Red diamonds mark the five sensor locations. The black regions around the transducers are the unpenalized exclusion zones from the regularization.}
...
\begin{figure}
  \begin{subfigure}[t]{0.48\linewidth}
    ...
    \caption{Experimental CFRP plate reconstruction of  $|\vec{\mathbf{d}}|$ using a 1--15 kHz bandpass filter with a 
50$\times$50 subsampled grid. Red diamonds mark the five sensor locations. The adhered metal-cube defect is clearly resolved, although its marker is omitted in the image for visual clarity (See Figure \ref{fig:Results_normal})} 
  ...
  \begin{subfigure}[t]{0.48\linewidth}
    ...
    \caption{Cross-sectional profiles along the x-axis through the reconstructed defect (left panel). Colored envelopes show slices with amplitude exceeding 0.5 times the maximum. The localization width is estimated from the FWH, yielding a half-width of approximately 1.4\,cm.}
    \label{fig:sideways_15khz_subsampled}
  \end{subfigure}
  \caption{Defect localization results in the 1--15\,kHz band using sparse transducer measurements. Left: full 2D reconstruction showing sub-wavelength localization of the defect. Right: cross-sectional analysis quantifying the localization width.}
...
\begin{figure}
  \begin{subfigure}{0.48\linewidth}
    ...
    \caption{Reconstruction of simulated case A5B using 5 ms period and 0--100 kHz bandwidth, demonstrating reduced background artifacts and sharper localization compared to Fig.~\ref{fig:Defect_A5B}.}
  ...
  \begin{subfigure}{0.48\linewidth}
    ...
    \caption{Single-transducer pulse-echo reconstruction of the simulated small defect from case A (Fig.~\ref{fig:Defect_A}), using 0--100 kHz bandwidth with 10\% RMS noise.}
    \label{fig:ECHO}
  \end{subfigure}
...
Experimental validation on the CFRP plate (Section~\ref{Results_Experimental}) demonstrates defect localization using the calibration-to-monitoring procedure and sparse measurements at the transducer locations. In the 1--50\,kHz reconstruction, the defect signature has a half-width of approximately 2\,cm. Relative to a wavelength of $\lambda_{50} \approx 8$\,cm at 50\,kHz (Figure~\ref{fig:Dispersion}), this corresponds to sub-wavelength localization; its origin and interpretation are discussed further in Appendix~\ref{Appendix_Sub-wavelentgh}. In addition, the very-low-frequency evaluation in the 1--15\,kHz band (Figure~\ref{fig:15khz_subsampled}) yields an estimated FWHM of $\approx 1.4$\,cm at a wavelength on the order of $\approx 16$\,cm, demonstrating strong localization even in the audible range.
\paragraph{}
The absolute pixel values of all results in section \ref{Results} should be interpreted as relative indicators of defect location rather than quantitative scattering amplitudes, since the regularization parameter $\lambda$ influences amplitude scaling. In future implementations, this dependence could be addressed by characterizing the amplitude change induced by regularization and by correcting for the density of grid points in the model.
\paragraph{}
...
\paragraph{}
The 1--15\,kHz experiments also highlight a practical conditioning effect: when the number of free defect parameters is too large relative to the information content of the data, the reconstruction can develop isolated high-amplitude pixels (Appendix~\ref{Appendix_15khz}). In this work, using the full $104\times104$ defect grid (10\,816 unknowns) with a measurement vector of length 300 produced mixed results, whereas restricting the inversion to a $50\times50$ subgrid (2\,500 unknowns) yielded a stable, compact peak. This behavior suggests that reconstruction quality depends on the joint ratio between spatial degrees of freedom and effective data constraints (set by bandwidth, period length, and sensor configuration), and/or by grid density and frequency. A more precise characterization of this trade-off would require systematic parameter studies in future work.
\paragraph{}
The experimental validation is comparable to the baseline simulation (Sections~\ref{Simulation_Results}, \ref{Results_Experimental}) with respect to the periodic excitation (2 ms repetition period), sparse measurements at 5 transducers, and band-limited processing in the 0--50 kHz range. The experimental setup deviates slightly by restricting the lower band to 1--50 kHz due to systematic measurement noise at very low frequencies and by using a different means to induce a small defect. This overall consistency supports using the simulation setup as representative of the method under comparable bandwidth and sparsity, while retaining the advantages of controlled ground truth and tunable noise.
\paragraph{}
...
\subsection{Low-frequency reconstructions on the full grid}\label{Appendix_15khz}
The main text (Fig.~\ref{fig:15khz_subsampled}) shows that, in the 1--15 kHz band, a compact defect signature can be obtained when the defect space is restricted to a $50\times50$ subgrid. For completeness, we here show what happens if the same data are inverted on the original $104\times104$ grid (10{,}816 unknowns).

From Figure~\ref{fig:Dispersion:b}, the corresponding wavelengths in the 1--15 kHz and 1--30 kHz bands are on the order of 16 cm and 10 cm, respectively, i.e. comparable to the plate size. In this regime, the number of free defect parameters becomes large relative to the information content of the data. The reconstructions in Figure~\ref{fig:15khz} still highlight the approximate defect region but are dominated by sparse isolated pixel-scale spikes, particularly on the right-hand side of the 1--15 kHz panel. Similar, though slightly less severe, behavior is observed in the 1--30 kHz case.

We interpret these features as a result of overparameterization at very low frequencies relative to grid size.

\begin{figure}
  \begin{subfigure}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{Figures/Results/15khz.png}
    \caption{1--15 kHz band, full $104\times104$ defect grid. Note the isolated high-amplitude pixels, especially on the right-hand side.}
  ...
  \begin{subfigure}{0.48\linewidth}
    ...
    \caption{1--30 kHz band, same $104\times104$ defect grid.}
    \label{fig:sideways_15khz}
  \end{subfigure}
  \caption{Low-frequency defect reconstructions on the full $104\times104$ grid. Compared to the subsampled 50$\times$50 result in Fig.~\ref{fig:15khz_subsampled}, the full-grid inversions show pronounced background artifacts and pixel-scale spikes, consistent with an overparameterized defect space at very low frequencies.}
  \label{fig:15khz}
\end{figure}
...
In the experimental example of section~\ref{Results_Experimental}, including higher-frequency data provides limited incremental benefit, even though the measured wave spectrum extends beyond 50 kHz. This is consistent with the 80 kHz central frequency of the transmitted pulses and is confirmed by measurements at multiple locations on the plate.
