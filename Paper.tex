...
\section{Results}
 We first present successful defect localization using the experimental setup described in Section \ref{Sec:Exp}. This is followed by an evaluation of the methods capabilities in locating different types of defects utilizing numerical data. 

\subsection{Results - Experimental Validation}\label{Results}

Figure~\ref{fig:Results_normal} shows the reconstructed defect vector $\vec{\mathbf{d}}$, visualized as a heatmap of its magnitude at each spatial grid point (LDV measurement location). Solving equation~\ref{full_system} yields a complex-valued coefficient for every grid point, interpreted here as a local reflection amplitude. In this experiment, $\vec{\mathbf{d}}$ contains $101\times104 = 10,816$ entries. For visualization, the 1D indexing is mapped back to the original 2D arrangement of the scan grid. Since the plot shows $|\vec{\mathbf{d}}|$, phase information is not displayed. The inversion itself is agnostic to geometry: it operates on the indexed set of points and is not intrinsically restricted to a 2D plane.

 The defect in Figure~\ref{fig:Results_normal} is localized near the target location (yellow square). Small black regions around each transducer (red diamonds) indicate exclusion zones, corresponding to the unpenalized region in the regularization (Section~\ref{Regularized inversion}) and masked in the visualization. Without this masking, these areas would exhibit large reconstructed values that primarily capture systematic experimental differences (e.g., pulse-amplitude changes between calibration and monitoring) rather than defect scattering.

 Overall, the $1\times1\times1\,\mathrm{cm}^3$ iron cube is successfully localized using only sparse measurements at the five transducer locations. The particle velocity data were bandpass filtered to 1--50 kHz and processed using the Tikhonov-regularized inversion described in Section~\ref{Regularized inversion}.

 Figure~\ref{fig:Results_side} provides cross-sectional views along the x-axis. Slices exceeding 0.6 times the maximum amplitude are retained and used to draw an envelope. Under this criterion, the defect signature exhibits a half-width of approximately 2 cm.

 A 15 kHz evaluation can be seen in Appendix \ref{Appendix_50khz_pulse}.

%Figure~\ref{fig:Results_normal} shows the reconstructed defect vector $\vec{\mathbf{d}}$, in which each pixel represents the absolute value at a spatial grid point corresponding to an LDV measurement location. Solving equation~\ref{full_system} assigns a reflection amplitude to each grid point, resulting in a vector containing $104\times104 = 10{,}816$ entries. For visualization, these entries are mapped back onto the two-dimensional plane according to their original spatial arrangement. The heatmap therefore represents the magnitude of the reconstructed scattering response at each location and may be interpreted as a relative reflection amplitude within the assumptions of the linear forward model. Although the reconstructed vector is complex-valued, only its magnitude is displayed for clarity. The inversion itself operates in index space and does not explicitly rely on spatial ordering.
%; spatial structure is introduced only during visualization, and the formulation is not inherently restricted to two-dimensional geometries.

%The defect in Figure~\ref{fig:Results_normal} is clearly localized near the target location (yellow square). Small black regions surrounding each transducer (red diamonds) correspond to exclusion zones defined by the unpenalized region in the regularization (Section~\ref{Regularized inversion}) and are masked in the visualization. Without masking, these regions would exhibit high amplitudes primarily caused by systematic experimental effects, such as differences in pulse amplitude between calibration and monitoring measurements. Consequently, these areas are excluded, as they do not represent physically meaningful scattering locations but arise from source–receiver coupling and experimental bias.

%The results demonstrate successful localization of the $1\times1\times1,\mathrm{cm}^3$ iron cube using sparse measurements from only five transducer locations. Here, localization refers to accurate recovery of the defect position, whereas the spatial extent of the reconstructed signature remains limited by the available bandwidth and measurement aperture. The particle-velocity data were bandpass filtered between 1 and 50 kHz and processed using the Tikhonov-regularized inversion described in Section~\ref{Regularized inversion}.

%Figure~\ref{fig:Results_side} presents cross-sectional views along the x-axis. Regions exceeding 0.6 times the maximum amplitude are retained, and their envelope is plotted to estimate the localization width. The threshold was chosen empirically to balance noise suppression and preservation of spatial extent. The reconstructed defect signature exhibits an approximate half-width of 2 cm.

%Figure~\ref{fig:Results_side} provides cross-sectional views along the x-axis. Slices exceeding 0.6 times the maximum amplitude are retained, and their envelope is plotted to estimate the localization width. This threshold identifies the localization width: the defect signature has a half-width of approximately 2 cm.

%A 15 kHz evaluation is provided in Appendix~\ref{Appendix_50khz_pulse}.
...
\subsection{Results - Simulations}\label{Appendix_Simulation_Results}\label{Simulation_Results}
The method was validated using the spectral element modelling suite Salvus \cite{mondaic}. We simulated elastic wave propagation on a 50 x 50 cm large and 9 mm thick rectangular aluminum plate, with defects modeled as localized reductions in density.

\textbf{Case A}: Figure \ref{fig:Defect_A} shows a tapered circular lower-density inclusion of 1.5 cm diameter with relative density 20\%. As the simplest case, it serves as a reference for comparison with the other results. The repetition period is 2 ms, with 5 sensors in a similar configuration, 20\% Gaussian RMS noise, and a 0--50 kHz low-pass filter matching the real-world example. A 50 kHz Ricker wavelet is used as the driving pulse instead of 80 kHz, which should not significantly change the results.

\textbf{Case A5B\_1}: Figure \ref{fig:Defect_A5B} contains two tapered inclusions of 1.5 cm and 5 cm diameter at relative density 0.2, using the same parameters as before. It showcases the ability of the method to discern multiple defects simultaneously and larger shapes, despite internally assuming single scattering. The reconstruction, however, shows degradation in the form of significant background artifacts, which appear when the scenario departs from cases well described by single scattering.

\textbf{Case A5B\_2}: In Figure \ref{fig:Simulation_A5B_100khz_results}, the same defect as in \ref{fig:Defect_A5B} is simulated with a 5 ms period and 0--100 kHz frequency range. The algorithm therefore has access to five times more temporal information (2 $\times$ higher frequency, 2.5 $\times$ longer period) than in the previous cases. We observe a marked increase in defect reconstruction fidelity and a decrease in background artifacts.  This case illustrates that when more temporal bandwidth and a longer listening window are available, the method can produce sharper images with less background, while a systematic analysis of these effects is deferred to \cite{WueestMScThesis}.

\textbf{Case ECHO}: Figure \ref{fig:ECHO} provides a proof of concept that the method can operate using only one sensor. In this case, a single transducer is active as emitter and simultaneously acts as the only sensor, essentially a pulse-echo configuration. The data are bandpass filtered to 0--100 kHz with a 5 ms period and 10\% RMS noise. All other parameters are the same as in the other simulation cases. The reconstructed defect closely resembles the small defect of Figure \ref{fig:Simulation_A5B_100khz_results}, rather than \ref{fig:Simulation_A_results}, indicating that localization accuracy is primarily determined by data frequency rather than the number of sensors and thus the amount of data. However, more complex defects such as \ref{fig:Simulation_A5B_Defect} (not shown here) are evaluated much worse in this echo configuration. It is more susceptible to noise and works significantly worse at lower frequencies. These findings align with the description of a similar problem in \cite{Touchscreen}, where increased sensor counts decrease "contrast" (called "background artifacts" here) but do not increase resolution as higher frequencies do. The problem statement is very similar to that of \cite{ebrahimkhanlou_acoustic_2017,One_Channel_reversal}, providing a close comparison to different approaches. Evaluation attempts using real world data did not yield interpretable results, which is in line with the simulations performing poorly as well with the same parameters, as the low frequency and period length offers very little information.
...
The reconstructed defect has a half-width of approximately 2 cm compared to a wavelength of $\lambda_{50} \approx 8$ cm at 50 kHz (Figure~\ref{fig:Dispersion:a}), yielding sub-wavelength localization. This is notably below the wavelength scale. The reasons for this property have prompted considerable discussion, which are discussed in \ref{Appendix_Sub-wavelentgh}.
\paragraph{}
The absolute pixel values in Figure~\ref{fig:Results_normal} should be interpreted as relative indicators of defect location rather than quantitative scattering amplitudes, as the regularization parameter $\lambda$ significantly influences amplitude scaling. Testing with extended frequency ranges showed that the 1--80 kHz band yields only modest improvements over 1--50 kHz, while extending to 1--150 kHz causes severe degradation. This likely stems from imperfect LDV realignments during defect measurements, an issue that should disappear entirely when using transducers for monitoring (Appendix~\ref{Appendix_Frequencies}). The unpenalized regions around transducers (Section~\ref{Regularized inversion}) suppress phantom defects by absorbing pulse variations locally, demonstrating the method's robustness to experimental imperfections.
\paragraph{}
The experimental defect is an adhered metal cube, which differs from realistic CFRP damage such as delaminations. Delamination detection has been explored with mixed results in \cite{WueestMScThesis}, with room for improvement. However, the cube defect serves as a useful proxy for contact-based applications, and the computational efficiency after calibration (Appendix~\ref{Appendix_Computation}) suggests viability for both frequent automated SHM and low-cost touchscreen implementations \cite{Touchscreen}.
\paragraph{}
Although not explicitly explored in this paper, previous simulation work documented in \cite{WueestMScThesis} indicated several limitations of the approach. Large defects with a relative size of order $\approx 0.1$ of the system become hard to resolve, likely because the single scattering approximation starts to break down. The reconstruction quality also depends on the defect shape, and while small defects can be localized well below the wavelength, their shape cannot be recovered if they are much smaller than the wavelength.


\section{Conclusion}\label{Conclusion}
This paper presented and experimentally validated an empirical, fully vectorized linear‑operator framework for inverse scattering and structural health monitoring. Using bonded, very cheap piezoelectric transducers and a one‑time LDV calibration scan, we constructed source–receiver impulse‑response operators on a discrete periodic time axis, modeled propagation as circular convolution diagonalized by the DFT, and posed defect localization as a Tikhonov‑regularized linear inverse problem. On an anisotropic CFRP plate, an adhered $1\,\mathrm{cm}^3$ iron cube was localized from sparse measurements at the transducer locations with sub‑wavelength resolution of about $2\,\mathrm{cm}$ at a minimum wavelength of about $8\,\mathrm{cm}$. Complementary simulations on an aluminum plate showed accurate localization of small inclusions, the ability to distinguish multiple defects, defect shapes, as well as a proof‑of‑concept operation in a single‑sensor echo configuration.

The framework is general and modular, since it does not rely on a detailed forward model and can in principle be directly applied to other defect models, sensing layouts, and wave‑bearing media as long as linearity and time invariance are given. Working effectively low frequency in the 1–50 kHz band, despite pulse‑shape drift and LDV misalignment, supports the central hypothesis that empirical operators can absorb many experimental imperfections while retaining sub-wavelength localization. At the same time, limitations remain, including the single‑scattering assumption, the simple delta‑like defect basis, and reliance on LDV for calibration. These point to natural future work toward richer scattering models, fully transducer‑based implementations, and practical SHM systems in which the one‑time calibration enables fast, low‑cost monitoring, possibly including touch‑like sensing applications.
...
On a standard laptop with 16\,GB RAM (about 9\,GB effectively usable) and an Intel Gen13 i7 CPU, practical limits are matrices of order $10^4\times10^4$ in Float32, with additional overhead from temporary work arrays. For the experimental CFRP case studied here ($\approx10^4$ defect parameters and $\approx10^3$ measurements), the one-time inversion of $\mathcal{L}_{\text{full}}$ takes about $17$\,s. The empirical impulse responses (Section~\ref{Empirical impulse responses}) are obtained in roughly 4\,s and scale linearly in grid size and measurement points, so they remain inexpensive compared to the inversion as the scale increases. The subsequent evaluations for new measurement vectors reduce to a single matrix--vector multiplication and take around 3--5\,ms in the presented case. From a computational perspective, this separation between a moderately expensive calibration step and very fast evaluation makes the method potentially viable as a low-cost, touchscreen application as in \cite{Touchscreen}. The evaluation does not have any memory requirements, potentially allowing frequent automated SHM on a simple system.
...
\end{figure}
\paragraph{}
Pushing the low‑frequency properties to the limit, we explore the 1--15 kHz band, which is the lowest range available in the experimental setup. From Figure~\ref{fig:Dispersion:b}, the corresponding minimum wavelength is on the order of 16 cm, comparable to the plate dimensions. The result therefore serves as a proof of concept for operation at much lower frequencies and provides an additional argument for sub‑wavelength localization capability. Below this range the method no longer yields discernible defect signatures, which may be caused by the wavelength approaching the plate size. Improved performance at such low frequencies would require more information, either by extending the repetition period (if low attenuation permits) or by increasing the number of sensors.

The result in Figure~\ref{fig:15khz} shows a clear but less clean localization of the defect, with substantial background artifacts and isolated grid points that reach very high amplitudes, particularly on the left side. The origin of these isolated peaks is unclear. They may be related to grid points with poor or missing measurements, which are present at all frequencies but appear most prominently in this low‑frequency setting.

\begin{figure}
  \begin{subfigure}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{Figures/Results/15khz.png}
    \caption{CFRP plate defect reconstruction using a 1--15 kHz bandpass filter. In this range the waves lie fully within the human audible band.}
    \label{fig:result_15khz}
  \end{subfigure}\hfill
  \centering
  \begin{subfigure}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{Figures/Results/15khzsection_scan_L_10.0_x.png}
    \caption{Cross-sectional profiles along the x-axis through the reconstructed defect. Colored envelopes show slices with amplitude exceeding 0.5 times the maximum. Some narrow noise peaks are hidden by the legend.}
    \label{fig:sideways_15khz}
  \end{subfigure}
  \caption{Defect localization in the very low 1--15 kHz band. The defect remains identifiable, but background artifacts and isolated high-amplitude pixels are more pronounced than at higher frequencies.}
  \label{fig:15khz}
\end{figure}


\subsection{Frequency content and dispersion}\label{Appendix_Dispersion_Curve}\label{Appendix_Frequencies}
...
A bigger concern may be that manually computing $dx/dt$ on Figure \ref{fig:Dispersion:a}, which corresponds more to the group velocity, yields a result of roughly 1700m/s.
...
The results shown in this paper use the simple delta basis. However, we can generalize the approach to have an arbitrary basis vectors, and multiple of those available at each grid location.
...
This approach is not used in this paper, as we continue using a delta-like simplifies defect. Experiments in simulation work so far has not yielded many useful bases that significantly increased performance.  However, this generalized approach is likely needed to increase the performance in detecting delaminations in CFRP, which seem to not act very delta-like, by empirically measuring the defect response of a delaminated spot \cite{WueestMScThesis}.



\subsection{Sensor Configurations}\label{Appendix_sensor_configs}
The sensor configuration used in this paper as described in section \ref{Operator assembly} are chosen as s [[1,2], [1,3], [1,4],
[1,5], [2,3], [2,4], [2,5], [3,4], [3,5], [4,5] ]. For the echo configuration in Figure \ref{fig:ECHO}, it is [[1,1]]. The choice of the 5-sensor system is motivated by avoiding symmetric sets to be included, such as [[1,2],[2,1]], as due to the reciprocity assumption, do not provide additional significant useful information. Testing confirmed this to be the case.  
\paragraph{}
In hindsight, it would likely be better to use a basis layout such as [[1,2],[1,3],[2,3],[2,4],[3,4],...] to treat all emitters equally importantly. This was  thought of too late, but testing afterwards revealed no noticeable improvements. Therefore, the results were kept.
\paragraph{}
The inclusion of a full set of measurements including self-measurements such as ([1,1],[2,2] etc.) should in theory improve the results, but testing shows a tendency to decrease the result quality, which is odd, given the single pulse cases \ref{fig:ECHO} do work, therefore the information should not be useless. 

\subsection{Sub-Wavelength localization discussions}\label{Appendix_Sub-wavelentgh}

and it remains challenging to provide a concise explanation, especially since it is difficult to demonstrate why something does \textit{not} occur. Based on the mathematical methodology used which does not rely on explicit spatial information, there appears to be no inherent restriction enforcing a wavelength limit. In contrast, other methods often have such restrictions, typically due to the need to reconstruct measured waves in space. For example, \cite{ebrahimkhanlou_acoustic_2017} describes a method with this limitation, though it is not explicitly discussed there.
\paragraph{}
