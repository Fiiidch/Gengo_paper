...
Structural Health Monitoring (SHM) of engineered components aims to localize and characterize defects. The importance of which depends on the cost of failure, which is especially relevant in aviation, hence many methods of performing SHM on flat structures has been explored and documented. Among the methods, Piezoelectric sensors are promising from a commercial point of view due to their low cost, however increased challenges are noted when monitoring CFRP components \cite{Aircraft_NDT}. Methods for CFRP specifically are explored in \cite{Current_Methods}.
...
Transducers are low-cost ceramic discs of $12\,\mathrm{mm}$ diameter. The drive is a periodic pulse train with a $2\,\mathrm{ms}$ repetition period, using the 80\,kHz Ricker wavelet as the pulse shape to enforce timing periodicity. Data are sampled at $625\,\mathrm{kHz}$ and low-pass filtered to the analysis band prior to operator estimation. Each grid location is acquired with 10 repeats; within each acquisition, a 5-pulse train (total duration $\approx10\,\mathrm{ms}$) is emitted. The five pulses are segmented and treated as distinct measurements to increase robustness of the empirical impulse-response estimation. Transducers were bonded using a hand-applied, temperature-activated adhesive; exact formulation and bond thickness were not recorded. It is not required, as the formalism only compared responses between points, so the coupling properties of the transducers are not important to know.
...
After calibration, a $1\,\mathrm{cm}^3$ iron cube is bonded to the plate to simulate a defect. During monitoring, we excite each transducer again but measure only at the transducer locations. In our setup, the LDV acquires these sparse responses at the transducer positions; in principle, the transducers themselves can act as receivers, enabling fully automated, remote NDT/SHM. See Appendix~\ref{Appendix_Piezoelectric} for additional discussion of piezoelectric sensing considerations.
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\linewidth]{Figures/Raw_data/150khz0.17ms.png}
  \caption{Snapshot of the CFRP plate excitation. TODO: Alle Sensor locations rein plotten}
...
\begin{figure}
  ...
  \begin{subfigure}{0.48\linewidth}
    ...
    \caption{Raw LDV space--time slice along the x-direction through the active transducer. }
