...
In both the simulations and the experiments, only the out-of-plane velocity component is used. The experimental setup employed an LDV capable of recording velocity in all three spatial directions. In principle, the method should be compatible with using any single component, or a combination of components (e.g., their sum), as the measured quantity. This option was not evaluated in the simulations. In the experimental data, the in-plane components exhibited low signal-to-noise ratios and many "dead" grid points with missing measurements for unknown systematic reasons, yielding non-interpretable results. Consequently, we could not verify that the method extends to longitudinal (in-plane) velocity components.

Exploring this aspect may be relevant for practical SHM implementations. If the hypothesis holds that the sum of all three velocity components is a valid input quantity, then for curved surfaces this could reduce the need to compute an out-of-plane component that varies with local surface orientation. This would enable a simplified workflow in which the three-component velocity at each grid point is provided directly to the algorithm without explicitly accounting for surface orientation. Further reading: \cite{WueestMScThesis}.
...
\end{figure}
\paragraph{}
To probe the low-frequency limit, we evaluate the 1--15 kHz band, which is the lowest functional range in the experimental setup. From Figure~\ref{fig:Dispersion:b}, the corresponding minimum wavelength is on the order of 16 cm. This case serves as a proof of concept for operation at substantially lower frequencies and provides an additional indication of sub-wavelength localization capability. Below this range, the method no longer yields discernible defect signatures, which may be caused by the wavelength approaching the plate size. Improved performance at such low frequencies would require additional information, for example by extending the repetition period (if low attenuation permits) or by increasing the number of sensors.

The result in Figure~\ref{fig:15khz} shows that the defect remains identifiable, but the reconstruction contains substantial background artifacts and isolated grid points that reach very high amplitudes, particularly on the left side. The origin of these isolated peaks is unclear; they may be related to grid points with poor or missing measurements, which are present at all frequencies but appear most prominently in this low-frequency setting.
...
\subsection{Behavior of experimental data at high frequencies}\label{Appendix_Frequencies}
