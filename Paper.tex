...
\textbf{Case ECHO}: Figure \ref{fig:ECHO} is a proof of concept that this method can work using only one sensor. In this case, only one sensor is active as emitter, and simultaneously acts as the only sensor, essentially a pulse-echo configuration. The data in this case is band passed to 100khz at 5ms period length, and only using 10\% RMS noise. Otherwise, the parameters are the same as the other simulation cases. While the result shows a very clean reconstruction competitive to the 5 sensor configuration in Figure \ref{fig:Defect_A} (Which has access to twice the amount of information), it must be said that this configuration is much more unstable than multiple sensor configurations. More complex defects such as \ref{fig:Simulation_A5B_Defect} are evaluated much worse, it is more susceptible to noise, and works significantly worse at lower frequencies. As a result, the evaluation attempts using real world data did not yield interpretable results.
