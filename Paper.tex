...
The method was validated using the spectral-element modelling suite Salvus \cite{mondaic}. We simulated elastic wave propagation on a 50\,cm $\times$ 50\,cm, 9\,mm-thick rectangular aluminum plate, with defects modelled as localized reductions in density. 

Figure~\ref{fig:Simulation_A_results} shows the reconstructed defect vector $\vec{\mathbf{d}}$, visualized as a heatmap of its magnitude at each spatial grid point (LDV measurement location). Solving equation~\ref{full_system} yields a complex-valued coefficient at every grid point, which is interpreted here as a local reflection amplitude. In this experiment, $\vec{\mathbf{d}}$ contains $100\times100 = 10,000$ entries. For visualization, the 1D indexing is mapped back to the original 2D arrangement of the scan grid. Since the plot shows $|\vec{\mathbf{d}}|$, phase information is not displayed. 

Figure \ref{fig:Defect_A} shows a tapered circular lower-density inclusion of 1.5 cm diameter with relative density 20\%. This baseline case serves as a reference for comparison with subsequent results. The  driving pulse is a 50kHz central frequency Ricker wavelet at a repetition period is 2 ms; the configuration uses 5 sensors, 20\% Gaussian RMS noise, and a 0--50 kHz low-pass filter.
...
\textbf{Case A5B\_1}: Figure \ref{fig:Defect_A5B} contains two tapered inclusions of 1.5 cm and 5 cm diameter at relative density 0.2, using the same parameters as before section\ref{}. This case demonstrates that the method can discern multiple defects and larger shapes, despite internally assuming single scattering. The reconstruction quality degrades through pronounced background artifacts, consistent with a scenario that departs from conditions well described by single scattering.
...
The approach is validated experimentally on the CFRP plate (Section~\ref{Results}), demonstrating defect localization using the calibration-to-monitoring procedure and sparse measurements at the transducer locations. In the experimental reconstruction, the defect signature has a half-width of approximately 2 cm, compared to a wavelength of $\lambda_{50} \approx 8$ cm at 50 kHz (Figure~\ref{fig:Dispersion:a}), indicating sub-wavelength localization. The origin and interpretation of this effect are discussed further in Appendix~\ref{Appendix_Sub-wavelentgh}.
\paragraph{}
The absolute pixel values in Figure~\ref{fig:Results_normal} should be interpreted as relative indicators of defect location rather than quantitative scattering amplitudes, since the regularization parameter $\lambda$ influences amplitude scaling.  This can be accounted for in future implementations, as the decrease in the result amplitued act predicatbly with regularization, as well as by accounting for the density of grid points in the model.
\paragraph{}
In the experimental dataset, extending the bandwidth from 1--50 kHz  to higher frequencies initially yield only marginally better results, and degrades severely at frequencies over 100 kHz. Since this property is not observed in the simulated cases, it indicates that high frequency data are more susceptible to errors caused by real world implementation properties.  Due to the ability of the method to work in low frequency regions, such problems are partially side stepped and is therefore robust for use in real world impementations. For further discussion, see Appendix\ref{Appendix_Frequencies}. 
\paragraph{}
The experimental validation is comparable to the baseline simulation (Sections~\ref{Simulation_Results}, \ref{Results Experimental}) with respect to the periodic excitation (2 ms repetition period), sparse measurements at 5 transducers, and band-limited processing in the 0--50 kHz range. The experimental setup deviates slightly by restricting the lower band to 1--50 kHz, due to systematic measurement noise at very low frequencies, as well as using different means to induce a small defect. The overall consistency between experiment and simulation supports using the simulation setup as a representative for the method under comparable bandwidth and sparsity, while retaining the advantages of controlled ground truth and tunable noise in the simulated setting.
\paragraph{}
The additional simulations (Section~\ref{Appendix_Simulation_Results}) provide practical lessons that complement previous results. Case A5B\_1 indicates that multiple and larger inclusions can be localized simultaneously, but that background artifacts increase as scenarios depart from the single-scattering approximation. Case A5B\_2 shows that increased temporal information (longer period and higher-frequency content) can improve reconstruction fidelity and reduce artifacts, acting as a hypothetical ideal case implementation. Case ECHO demonstrates proof-of-concept localization with a single transducer. 
\paragraph{}
Limitations remain: Large defects with relative size $\approx 0.1$ become difficult to resolve when single scattering breaks down; and while small defects can be localized well below the wavelength, accurate shape recovery is not expected when they are much smaller than the wavelength \cite{WueestMScThesis}. More fundamentally,  the experimental defect is an adhered metal cube, and thus differs from realistic CFRP damage such as delaminations. However, the cube defect serves as a useful proxy for contact-based applications, and the computational efficiency after calibration (Appendix~\ref{Appendix_Computation}) suggests viability for both frequent automated SHM and low-cost touchscreen implementations \cite{Touchscreen}.
...
\subsection{Frequency content and dispersion}\label{Appendix_Dispersion_Curve}
Figures \ref{fig:Dispersion} suggest phase velocities that increase with frequency and reach values of order 7km/s at the upper end of the measured band. A theoretical Lamb-mode estimate\cite{Lamb_wave_Calculator} using rough CFRP parameters places the high-frequency A0 branch near ~3.8 km/s. The theoretical Lamb-mode calculation is only a rough ballpark because it assumes isotropic, homogeneous material properties and does not account for the plate's anisotropy or layup. Hence, discrepancies between the two are to be expected and can be substantial. Neither approach alone provides a definitive ``ground truth.'' The deviation could indicate a systematic error in computing, the code to which can be found in \cite{WueestMScThesis}. 

For the purposes of this paper we only use phase-velocity/wavelength estimates as a rough reference to assess whether the observed localization is below the order‑of‑magnitude wavelength. At 50 kHz, the corresponding wavelengths are on the order of a few centimeters, and the observed localization ($\approx$2 cm) remains smaller than both the theoretical estimate at 4cm, or the the experimental result at 8cm. 


\subsection{Behaviour of experimental data at high frequencies}\label{Appendix_Frequencies}
